...
function ping($host) {
  ...
  echo '<body>';
  echo '<table>';
  echo '<tr><th>Num</th><th>Device</th><th>Interface</th><th>IP address</th><th>Ping status</th>' . PHP_EOL;
  foreach ($ips as $num => $ip) {
    $ping = (ping($ip)) ? 'ping OK' : 'NO ping';
    $pingclass = ($ping == 'ping OK') ? 'online' : 'offline';
    echo '<tr><td>' . ($num + 1) . '</td><td>' . $name[$num] . '</td><td>' . $interface[$num] . '</td><td>' . $ip . '</td><td class="' . $pingclass . '">' . $ping . '</td></tr>' . PHP_EOL;
  }
